...
rLogin = r"INFO\s%s\s\"(?P<isre>RE)?LOGIN\s%s\s%s\s%s\s%s\s%s\s%s\s%s\"" % (rDateTime,rUid,rIP,rVersion,rPlatform,rDevice,rPayload,rExt)
...
def parseLogin(logline):
    ...
    if matchs is not None:
        ...
        print("%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (time,uid,isre,ip,country,proivnce,city,ctx,platform,device,date))
        return True
    return False
